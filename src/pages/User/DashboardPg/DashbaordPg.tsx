...
const DashbaordPg = () => {
    ...
    const DashItems = [
        {
            ...
            amount:`$ ${Number(data?.tDeposit).toLocaleString()}`,
        },
        {
            id: 2,
            title: "Withdrawn",
            icon: FaCloudMoonRain,
            amount: `$ ${Number(data?.twithd).toLocaleString()}`
        },
        {
            id: 3,
            title: "Total Stakes",
            icon: BiDollarCircle,
            amount: data?.staked.length
        },
        {
            id:4,
            title: "Total Profit",
            icon: FaTrademark,
            amount: `$ ${Number(data?.tProfit).toLocaleString()}`
        }
    ]
    useEffect(() => {
  ...
  return (
    <div className="scroll-smooth dark:bg-[#1a1d2c]">
        ...
        <div className="flex flex-row gap-4 items-center text-neutral-700 font-[500]">
            ...
            <div className={`py-2 px-4 rounded-lg ${data?.verified == true ? "bg-green-100" : data?.verified == false ? "bg-red-100" : ""}  w-fit`}>
                <p className="font-[Jost] capitalize">{data?.verified == true ? "Verified": data?.verified == false ? "You're not verified" : ""}</p>
            </div>
        </div>  
        <div className="border border-neutral-100 dark:border-neutral-700 grid grid-cols-1 md:grid-cols-2 justify-between items-center px-6 py-4 rounded-xl mt-4 dark:bg-[#2a3042]">
        ...
        <div className="border border-neutral-100 dark:border-neutral-700 px-6 py-4 rounded-xl mt-4 flex flex-row items-center justify-center gap-4 dark:bg-[#2a3042]">
        ...
        <FcNeutralTrading size={24} />
                ...
                <div className="w-full">
                    ...
                    <input type="range" name=""  id="" value={data?.tradProg} className="text-green-500 w-full outline-none" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 mt-4 gap-4 md:gap-8">
            {DashItems.map((item, index) => (
             <div className="border border-neutral-100 dark:border-neutral-700 rounded-xl bg-primary bg-opacity-5 p-4 flex flex-row justify-between items-start dark:bg-[#2a3042]" key={index}>
                <div>
                    <h3 className="text-neutral-700 dark:text-neutral-100 font-[600] font-[Jost]">{item.title}</h3>
                    <p className="font-[600] font-[Jost]">{item.amount}</p>

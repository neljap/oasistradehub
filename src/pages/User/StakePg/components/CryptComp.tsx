...
const CryptComp = () => {
    ...
    const {inputStkCrypto, handleStkCrypto, filteredStkCrypto} = useContext(StakeContext);

  return (
    <div>
      <div className="my-4">
        <input type="text" value={inputStkCrypto} onChange={handleStkCrypto} placeholder="Search Cryptos eg. 'BTC'" className="w-full border border-neutral-200 rounded-xl p-2"/>
      </div>
    ...
    {filteredStkCrypto.map((item: any) => (
        ...
        <div className="bg-neutral-200 rounded-lg px-2 py-1">
          ...
          <button className="bg-primary w-full text-white py-3 rounded-xl" onClick={() => setStakeOpen(true)}>Stake {item.coin}</button>
        </div>
